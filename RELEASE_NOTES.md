...
- Remediated the remote-sprite PM seed example so copyable tasks do not force `kanban-worker`, `software-factory`, or `remote-sprite-development` by default; remote-sprite startup use now requires exact target-profile loadability verification or an inlined contract fallback.
...
- Added PM forced-skill guidance: PM-created Kanban tasks default to no task-level forced skills, must not force role/project/built-in skills by default, and may specify a startup skill only with target-profile loadability evidence, a startup requirement, and fallback/remediation behavior.
